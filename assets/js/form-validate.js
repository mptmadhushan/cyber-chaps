function valid_datas(f) {
  if (f.name.value == "") {
    ...
    jQuery.ajax({
      url: "mail.php",
      type: "post",
      data: jQuery("form#CyberChaps-contact").serialize(),
      complete: function (data) {
        jQuery("#form_status").html(data.responseText);
        jQuery("#CyberChaps-contact")
          .find("input,textarea")
          .attr({ value: "" });
        jQuery("#CyberChaps-contact").css({ opacity: 1 });
        jQuery("#CyberChaps-contact").remove();
      },
    });
    jQuery("#form_status").html(
      '<span class="loading">Sending your message...</span>'
    );
    jQuery("#CyberChaps-contact").animate({ opacity: 0.3 });
    jQuery("#CyberChaps-contact")
...
function notice(f) {
  jQuery("#CyberChaps-contact").find("input,textarea").css("border", "none");
